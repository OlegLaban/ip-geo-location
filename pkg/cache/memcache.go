...
func (cs *CacheService) Get(key string) ([]byte, error) {
	...
	if !ok {
		...
		return []byte{}, errors.Join(ErrCantGetDataFromCache, fmt.Errorf("not found data via key %s", key))
	}
...
func (cs *CacheService) GetWithCallback(key string, callback func() ([]byte, error)) ([]byte, error) {
	if !cs.Has(key) {
		...
		err = cs.Set(key, val)
		if err != nil {
			cs.logger.Error("can`t set data to cache", "err", err)
			return val, errors.Join(ErrSetDatToCache, err)
		}
	}
